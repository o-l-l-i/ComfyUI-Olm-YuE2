...
class CachedNAR:
    ...
    def _attention(self, q, k, v, causal=False):
        return (self.model.nar_attention or attention)(q, k, v, causal=causal, backend=self.backend, query_chunk_size=self.query_chunk_size)
...
def synthesize(model, prefix: Sequence[int], codec: Sequence[int], seed: int,
    ...
    for chunk_index, chunk in enumerate(chunks):
        ...
        with offload_context(model) if offload_context is not None else _offload_ar(model, offload_ar):
            try:
                progress = None
                if on_progress is not None:
