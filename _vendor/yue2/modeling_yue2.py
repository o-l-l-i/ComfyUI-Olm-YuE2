...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            out = self.sdpa(q, k, v, attn_mask=attention_mask[..., :k.shape[2]])
        else:
            out = self.sdpa(q, k, v, is_causal=(T > 1 and k.shape[2] == T))
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if ar_mask is not None:
            ...
            core_out = self.self_attn.sdpa(query, key, value, attn_mask=attention_mask)
